...
def load_sentinel_data(
    ...
    """
    items = [item for year in years if (item := get_best_item_for_year(catalog, year, bbox, collection, max_cloud_cover)) is not None]
    datasets = []
    for item in items:
        ds = xr.open_dataset(item.assets[asset].href, engine="eopf-zarr", chunks=chunks)
...
def load_sentinel_scl(
    ...
    scl_datasets = []
    for item in items:
        scl_z = zarr.open(item.assets[asset].href, mode="r")
        scl_da = xr.DataArray(
